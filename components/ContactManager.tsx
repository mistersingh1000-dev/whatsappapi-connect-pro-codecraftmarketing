...
export default function ContactManager() {
  ...
  return (
    <div className="container-px py-12">
        ...
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            ...
            <span className="rounded-full bg-emerald/12 px-3 py-1.5 text-emerald">
              {optedInCount} marketing opted-in
            </span>
          </div>
        </div>
      ...
      {success && (
        <div className="mb-4 rounded-xl border border-emerald/30 bg-emerald/[0.06] p-3 text-sm text-emerald">
          {success}
        </div>
      )}
          ...
          <label className="mt-5 flex items-start gap-3 rounded-2xl border p-4" style={{ borderColor: "var(--line)" }}>
            ...
            <span>
              ...
              <span className="muted mt-1 block text-xs leading-relaxed">
                Only enable this when you have a valid opt-in/consent record. Campaigns automatically exclude contacts without consent.
              </span>
            </span>
          </label>

          <button onClick={addContact} className="btn-primary mt-5 w-full sm:w-auto">
            Save Contact
          </button>
        </div>
      )}
      ...
      <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {!contact.doNotMessage && (
                    <button
                      ...
                      onClick={() => setConsent(contact, !eligible)}
        ...
        {contacts.length === 0 && (
          <div className="card p-12 text-center">
            ...
            <p className="muted mt-2 text-sm">Add a contact or receive an inbound WhatsApp conversation.</p>
          </div>
        )}
      </div>
    </div>
