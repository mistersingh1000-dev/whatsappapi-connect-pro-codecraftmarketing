...
export default function TemplateManager() {
  ...
  return (
    <section className="container-px py-12">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            ...
            <span className="rounded-full bg-emerald/12 px-3 py-1.5 text-emerald">
              {approved} approved
            </span>
          </div>
        </div>
      ...
      {success && (
        <div className="mb-5 rounded-xl border border-emerald/30 bg-emerald/[0.06] p-4 text-sm text-emerald">
          {success}
        </div>
      )}
      ...
      {showCreate && (
        <div className="card mb-8 p-6">
          ...
          <p className="muted mt-1 text-xs leading-relaxed">
            Meta reviews the template. Submission does not guarantee approval. Use clear, policy-compliant wording.
          </p>
          ...
          <button disabled={creating || !form.name || !form.body} onClick={createTemplate} className="btn-primary mt-5 disabled:opacity-50">
            {creating ? "Submitting to Meta…" : "Submit for Meta Review"}
          </button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="card p-10 text-center">
          <p className="font-medium">No templates returned by Meta</p>
          <p className="muted mt-2 text-sm">Create a template here or in WhatsApp Manager, then refresh.</p>
        </div>
      ) : (
              ...
              <div key={`${t.id || t.name}-${t.language}`} className="card p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="muted mt-1 text-xs">{t.category} · {t.language}</p>
                  </div>
                ...
                {ok && (
                  <Link href="/dashboard/campaigns" className="mt-4 inline-flex text-sm font-medium text-emerald hover:underline">
                    Use in campaign →
                  </Link>
                )}
              </div>
